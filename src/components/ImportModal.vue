 ...
 <div class="modal-content modalbg modaltop">
   ...
   <div class="modal-body">
     ...
     <table class="table borderless table-hover table-space">
       ...
       <td  colspan="2" class="" >
           <b-form-input type="text" class="edit" v-model="item.roleType.$model" size="15"/>
             <div class="error-message" v-if="!item.roleType.required">Role is required.</div>
       </td>
       <td>
    ...
    remove(index){
      ...
      var data=[];
        this.details.forEach(e => {
          if(e.username!='' && e.contact!='' && e.password!='' && e.roleType!=''){
            data.push(e);
          }
        });
